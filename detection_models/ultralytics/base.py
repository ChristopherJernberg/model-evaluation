...
class UltralyticsModel(ABC):
    ...
    def predict(self, frame: np.ndarray, verbose: bool = VERBOSE, conf: float = CONFIDENCE_THRESHOLD, 
                iou: float = IOU_THRESHOLD) -> list[tuple[float, float, float, float, float]]:
